...
import { useExpandableQuery } from "@/hooks/table/useExpandable";
...
>) => {
    const shouldShowExpandable = config.condition
        ? config.condition(row)
        : true;
...
export const ExpandableContent = <TData extends Record<string, any>, TSubData>({
    ...
    useEffect(() => {
        ...
        const transformed = config.transform
            ...
            : fetchedData;
        setSubData(transformed as TSubData);
    }, [fetchedData, config.transform]);
