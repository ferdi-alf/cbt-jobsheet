...
class TestController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Test::query()
            ->select(['id', 'materi_id', 'type', 'title', 'duration_minutes',
                      'is_score_visible', 'created_by', 'created_at', 'start_at', 'end_at'])
            ->with([
        ...
        if ($user->role === 'guru') {
            // Eksplisit dari DB — tidak pakai $user->guruProfile
        ...
        $items = collect($paginator->items())->map(function (Test $t) {
            return [
                ...
                'created_by'  => $t->creator ? [
                    ...
                    'role'      => $t->creator->role,
                    'full_name' => $t->creator->guruProfile?->full_name,
                ] : null,
    ...
    public function store(TestStoreRequest $request)
    {
        ...
        $materi = Materi::with(['kelas:id,name', 'mapel:id,name'])->findOrFail($data['materi_id']);
    ...
    public function questions(Request $request, Test $test)
    {
        ...
        $query = TestQuestion::where('test_id', $test->id)->orderBy('order');
        ...
        $items = collect($paginator->items())->map(fn (TestQuestion $q) => [
            'id'             => $q->id,
            'order'          => (int) $q->order,
            'question'       => $q->question,
            'options'        => [
    ...
    public function overview(Request $request, Test $test)
    {
        ...
        $top5 = TestAttempt::where('test_id', $test->id)
            ->whereNotNull('score')
            ->orderByDesc('score')
            ->limit(5)
        ...
        $above80 = TestAttempt::where('test_id', $test->id)->whereNotNull('score')->where('score', '>=', 80)->count();
        $below80 = TestAttempt::where('test_id', $test->id)->whereNotNull('score')->where('score', '<',  80)->count();
    ...
    public function attempts(Request $request, Test $test)
    {
        ...
        $query = TestAttempt::where('test_id', $test->id)
            ->with([
    ...
    {
        if ($type !== 'pretest') return;

        $exists = Test::where('materi_id', $materiId)
            ->where('type', 'pretest')
